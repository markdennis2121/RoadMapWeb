...
function App({ session }) {
  ...
  const openProfile = () => {
    ...
    document.body.appendChild(container);
    const root = createRoot(container);
    root.render(<ProfileModal session={session} onClose={() => { root.unmount(); container.remove(); }} />);
  };
  return <DashboardShell session={session} data={data} stats={stats} learningProgress={learningProgress} filter={filter} search={search} setFilter={setFilter} setSearch={setSearch} filteredCategories={filteredCategories} filteredProjects={filteredProjects} update={update} setModal={setModal} onProfile={openProfile} onSignOut={() => supabase.auth.signOut()} itemType={itemType} setItemType={setItemType} newTitle={newTitle} setNewTitle={setNewTitle} categoryId={categoryId} setCategoryId={setCategoryId} addItem={addItem} reset={reset} modal={modal} />;
}
